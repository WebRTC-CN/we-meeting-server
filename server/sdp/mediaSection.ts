...
declare type ProducerCodecOptions ={
	opusStereo?: boolean;
	opusFec?: boolean;
	opusDtx?: boolean;
...
declare type PlainRtpParameters =
{
	ip: string;
	ipVersion: 4 | 6;
	port: number;
...
export class AnswerMediaSection extends MediaSection
{
	...
	)
	{
		...
		switch (offerMediaObject.type)
		{
            ...
            if (!offerCodec) {
              console.error(`codec: "${codec.mimeType}" width playload: "${codec.payloadType}" not found in offerRtpParameters.codec`);
            } else {
        ...
        if (answerRtpParameters.headerExtensions) {
          ...
          if (codec.rtcpFeedback) {
            for (const fb of codec.rtcpFeedback)
            {
              this._mediaObject.rtcpFb.push(
				...
				this._mediaObject.ext = [];
        if (offerRtpParameters.headerExtensions) {
